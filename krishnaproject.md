...
| Database (Node.js) | SQLite (sql.js) | In-memory + file |
...
└── Backend/
    ...
    │       │   ├── src/index.js
    │       │   ├── package.json
    │       │   └── data/auth.sqlite     # Auto-created on first run
...
|----------|---------|----------|
| `AUTH_DB_FILE` | SQLite database file path | No (default: `./data/auth.sqlite`) |
...
### Database issues / corrupt data
**Fix:** Delete the database file and restart the backend (it auto-creates with seed data):
```bash
rm Backend/microservices/services/auth-service/data/auth.sqlite
# Restart the backend
```
